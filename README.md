...
10. [Lessons Learned & Challenges](#10-lessons-learned--challenges)
11. [팀](#11-팀)
...
│                                                                  │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌────────────────┐  │
│  │  Nginx   │  │ Frontend │  │ Backend  │  │      ML        │  │
│  │ (80/443) ├─▶│ Next.js  │  │ FastAPI  │  │    FastAPI     │  │
│  │ SSL 종료 ├─▶│  :3000   │  │  :8000   │  │     :8001      │  │
│  │ 라우팅   │  │          │  │ 인증/인가 │  │ 예측·스캐너    │  │
│  └──────────┘  └──────────┘  │ CRUD     │  │ 기술지표       │  │
│                               └────┬─────┘  └───────┬────────┘  │
│  ┌──────────────────────────────┐  │         ┌──────▼──────┐    │
│  │   Redis (Docker, :6379)      │◀─┼─────────│celery_worker│    │
│  │   메시지 브로커 + 캐시        │  │         └─────────────┘    │
│  └──────────────────────────────┘  │                             │
└───────────────────────────────────┬┘─────────────────────────────┘
                                    │
...
| ------------ | ------------------------- | ---------------------------- | ------------ |
| 컴퓨팅       | ECS Fargate ~$80-100/월   | EC2 t3.medium ~$33/월        | ~$67/월      |
| DB           | RDS Multi-AZ ~$100-150/월 | RDS t3.micro 단일AZ ~$16/월  | ~$119/월     |
| 스토리지     | —                         | EBS 20GB + RDS 20GB ~$4/월   | —            |
| Redis        | ElastiCache ~$18/월       | EC2 내 Docker ~$0            | ~$18/월      |
| 로드밸런서   | ALB ~$22/월               | Nginx on EC2 ~$0             | ~$22/월      |
| **합계**     | **~$220-300/월**          | **~$53/월**                  | **~$227/월** |

> **t3.small (2GB) 대신 t3.medium (4GB)을 선택한 이유**: OS(~300MB) + PyTorch CPU + Celery Worker + 6개 컨테이너를 합산하면 평상시 ~1.9GB, S&P 500 스캔 시 순간적으로 3GB 이상 소비합니다. t3.small(2GB)로는 스캔 실행 중 OOM(Out of Memory) 킬이 발생할 위험이 높아 t3.medium을 채택했습니다.

> AWS 프리 티어(t3.micro, 1GB RAM) 활용 시 OS + 6개 컨테이너 합산이 이미 1GB를 초과하므로 ML 기능을 포함한 정상 운영이 불가합니다.
...
| 레이어    | 기술                         | 선택 이유                                                                                    |
| --------- | ---------------------------- | -------------------------------------------------------------------------------------------- |
| Frontend  | Next.js 16 (TypeScript)      | App Router + SSR, EC2 Docker Compose로 배포, 기존 Streamlit UI를 대시보드로 재구현하기 위한 최적 선택 |
...
```
StockPriceAI/
├── frontend/                          # Next.js + TypeScript (EC2 Docker 배포)
│   └── src/
...
> 자동화 계획은 [docs/P01_CICD_ROADMAP.md](docs/P01_CICD_ROADMAP.md)를 참고하세요.
