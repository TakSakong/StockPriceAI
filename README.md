...
- 데이터 저장: JSON 파일 → **PostgreSQL(RDS)** 마이그레이션 예정
...
| ---------- | ------------------------- | --------------------------- | ------------ |
| 컴퓨팅     | ECS Fargate ~$80-100/월   | EC2 t3.small ~$15/월        | ~$85/월      |
...
> AWS 프리 티어(EC2 t2.micro + RDS t3.micro, 12개월) 활용 시 **사실상 $0**에 운영 가능
...
| 레이어    | 기술                         | 선택 이유                                                                                    |
| --------- | ---------------------------- | -------------------------------------------------------------------------------------------- |
| Frontend  | Next.js 14 (TypeScript)      | App Router + SSR, Vercel 무료 배포, 기존 Streamlit UI를 대시보드로 재구현하기 위한 최적 선택 |
...
│   └── app/
│       ├── api/v1/endpoints/          # auth, stocks, watchlist, scanner, predictions
...
│       ├── models/                    # predictor.py, xgboost_model.py, lstm_model.py
│       ├── services/                  # fetcher, technical, sentiment, scanner, charts
...
│
├── .github/workflows/
│   ├── ci.yml                         # PR 검증 (lint + type-check + pytest)
│   └── cd.yml                         # main 병합 시 EC2 SSH 자동 배포
│
...
- (백엔드/ML 코드 직접 수정 시) Python 3.10+, Poetry
- (프론트엔드 코드 직접 수정 시) Node.js 18+
...
초기 설계에서 ECS Fargate + ALB + ElastiCache + RDS Multi-AZ 조합을 검토했으나, 4인 팀 프로젝트에 **월 ~$250 절감** 가능한 EC2 + Docker Compose 조합으로 축소했습니다. 학교 프로젝트 규모에서 오버스펙 아키텍처는 운영 복잡도만 높인다는 것을 확인했습니다.
