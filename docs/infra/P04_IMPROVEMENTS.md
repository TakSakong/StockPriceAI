...
> Phase 1~2 (AWS 구축 + CD 자동화)이 안정화된 이후 적용하는 중기 과제들입니다.  
> 처음부터 한꺼번에 하려 하지 말고, 기본 배포가 잘 돌아가는 것을 확인한 뒤 순서대로 적용하세요.

---

## Phase 3 — HTTPS 적용 (보안 연결)

> **선행 조건**: 도메인을 구매하고, DNS A 레코드를 EC2 Elastic IP로 연결 완료
...
```
yourdomain.com  →  EC2_Elastic_IP
```

DNS 변경이 전 세계에 전파되는 데 최대 24시간이 걸릴 수 있습니다. 아래 명령으로 연결을 확인합니다:

```bash
nslookup yourdomain.com
# EC2 Elastic IP가 출력되면 연결 완료
```
...
Let's Encrypt 인증서는 **90일마다 만료**됩니다. 만료되면 브라우저에 "연결이 안전하지 않음" 경고가 표시됩니다.

갱신 시 certbot은 다시 80번 포트를 열어 도메인 소유를 검증하는데, 그 시점에 nginx가 이미 80번 포트를 점유하고 있습니다. 이를 해결하려면 갱신 전 nginx를 잠깐 내리고, 갱신 후 다시 올려야 합니다.

아래 명령으로 크론을 등록합니다 (nginx 중단 시간 ~10초):

```bash
# 크론 편집기 열기 (nano가 기본 편집기로 열립니다)
sudo crontab -e
```

편집기가 열리면 맨 아래에 아래 한 줄을 추가하고 저장합니다 (nano: `Ctrl+O` → `Enter` → `Ctrl+X`):

```
0 3 1 * * certbot renew --quiet --standalone --pre-hook "docker compose -f /app/docker-compose.yml -f /app/docker-compose.prod.yml stop nginx" --post-hook "docker compose -f /app/docker-compose.yml -f /app/docker-compose.prod.yml start nginx"
```

| 옵션 | 의미 |
|------|------|
| `0 3 1 * *` | 매월 1일 새벽 3시에 실행 |
| `--standalone` | certbot이 직접 80번 포트를 열어 검증 |
| `--pre-hook` | 갱신 시도 **전** 실행 — nginx를 멈춰 80번 포트를 비워줌 |
| `--post-hook` | 갱신 완료 **후** 실행 — 갱신 성공·실패 무관하게 항상 nginx를 재시작 |

> **검증**: `sudo certbot renew --dry-run` 으로 실제 갱신 없이 테스트할 수 있습니다.

### 3-4. nginx.conf HTTPS 설정으로 교체

`infra/nginx/nginx.conf`를 아래 내용으로 **전체 교체**합니다. (`yourdomain.com`을 실제 도메인으로 교체)
...
> **인증서 파일 경로 (`/etc/letsencrypt/...`)가 nginx 컨테이너 안에서 보이는 이유**: `docker-compose.prod.yml`의 nginx 서비스에 `/etc/letsencrypt:/etc/letsencrypt:ro` 볼륨이 이미 설정되어 있어서, EC2 호스트의 인증서 파일을 컨테이너 안에서 그대로 읽을 수 있습니다.

### 3-5. 전체 스택 재시작 및 nginx 이미지 재빌드

nginx.conf를 수정했으므로 nginx 이미지를 다시 빌드해야 합니다.

```bash
cd /app
git pull origin main   # 수정된 nginx.conf 받기

# nginx만 재빌드 후 재시작
docker compose -f docker-compose.yml -f docker-compose.prod.yml \
  up -d --no-deps --build nginx
```

### 3-6. 프론트엔드 환경변수 URL을 HTTPS로 업데이트
...
### 4-1. Backend 테스트를 PostgreSQL로 전환
...
### 5-3. Rolling Restart (서비스 중단 없는 배포)
