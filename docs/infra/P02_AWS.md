...
> **다음 단계**: [P03_CD_SETUP.md](./P03_CD_SETUP.md)에서 GitHub 자동 배포를 연결합니다.
...
**AWS 콘솔** 접속 → 우측 상단 리전(지역)이 원하는 곳인지 확인 → 검색창에 **EC2** → **Launch Instance** 클릭
...
> **SSH를 전체 허용하면 안 되는 이유**: `0.0.0.0/0`(전체 허용)으로 설정하면 전 세계 해커들이 로그인을 시도합니다. 반드시 내 IP만 허용하세요. 나중에 내 IP가 바뀌면 보안 그룹 설정에서 업데이트하면 됩니다.

**Launch Instance** 클릭 → EC2 생성 완료!

생성 후 EC2 목록에서 해당 인스턴스를 클릭해 **퍼블릭 IPv4 주소**를 메모해 두세요. 이후 단계에서 계속 씁니다.
...
# EC2에 SSH 접속 (EC2_퍼블릭_IP를 1단계에서 메모한 IP로 교체)
ssh -i /path/to/your-key.pem ubuntu@<EC2_퍼블릭_IP>
```

> **접속이 안 된다면?** 보안 그룹에서 SSH(22) 포트가 현재 내 IP로 허용되어 있는지 확인하세요. 공유기를 재시작하거나 장소가 바뀌면 IP가 변경될 수 있습니다. AWS 콘솔에서 보안 그룹 → 인바운드 규칙 수정 → SSH 소스를 `My IP`로 다시 설정하면 됩니다.
...
# EC2 퍼블릭 IP로 교체 (나중에 도메인이 생기면 도메인으로 변경)
...
앱이 잘 동작한다면 이 단계 완료입니다! 다음은 [P03_CD_SETUP.md](./P03_CD_SETUP.md)에서 자동 배포를 설정합니다.
