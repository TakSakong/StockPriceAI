...
### 📊 현재 구축 수준
```
CI (지속적 통합)  █████████▌  95%  (프론트엔드 vitest·통합테스트 추가 완료, PG 연동 미비)
...
* ~~**[GAP-CI-2] 프론트엔드 테스트 부재**~~ → **해결**: vitest + React Testing Library가 설치되어 5개 테스트 파일이 존재하며, `ci.yml`의 frontend 잡에 `npm run test` 단계가 포함되어 있습니다.
* ~~**[GAP-CI-3] Docker 이미지 빌드 검증 누락**~~ → **해결**: `ci.yml`에 `docker` job이 추가되어 backend/ml/frontend 모두 `--target production` 빌드를 검증합니다.
* ~~**[GAP-CI-4] 서비스 간 연동 검증 부재**~~ → **해결**: `ci.yml`에 `integration` job이 추가되어 docker compose 전체 기동 후 `test_local.sh --skip-ml`로 e2e 연동을 검증합니다.

#### ⚠️ CI 개선이 필요한 부분 (CI Gaps)
* **[GAP-CI-1] Backend 테스트가 SQLite를 사용**: psycopg2를 우회하여 SQLite in-memory DB로 테스트하므로 PostgreSQL 고유 쿼리 사용 시 프로덕션 버그를 예방하기 어렵습니다.
...
* **RDS**: PostgreSQL 16 (db.t3.micro), 퍼블릭 액세스 차단 (EC2 보안 그룹에서만 5432 허용)
      ...
      - name: SSH into EC2 and deploy
        ...
        with:
          ...
          script: |
            ...
            # deploy.sh 내부에서 git pull → docker compose up 을 순서대로 실행함
```
...
* ~~**Frontend 테스트 도입**~~ ✅ **완료**: vitest + React Testing Library 설정 완료, 5개 테스트 파일 작성, `ci.yml` frontend 잡에 `npm run test` 포함.
* ~~**Docker 빌드 사전 검증**~~ ✅ **완료**: `ci.yml`에 `docker` job이 추가되어 backend/ml/frontend 모두 production 타겟 빌드를 검증합니다.
* ~~**서비스 간 연동 검증**~~ ✅ **완료**: `ci.yml`에 `integration` job 추가, docker compose 전체 기동 후 e2e 스크립트로 연동 검증.
